...
class Colony:
    ...
    def __init__(self):
        ...
        self.screen, self.xmax, self.ymax, self.interpreter, self.archOS = \
                                    disputil.InitDisplay(settings.display_size[0], settings.display_size[1],
                                                         hideMouse=False, icon=settings.icons_folder + settings.icon, caption=settings.caption)
    ...
    def intro(self):
        ...
        icon = disputil.loadIcon("Virus1_Intro", settings.icons_folder)
        ...
        self.screen.blit(icon, (self.xmin - int(self.xmax*80/1280), self.ymax - int(self.ymax*180/800)))

        icon = disputil.loadIcon("Virus2_Intro", settings.icons_folder)
        icon = pygame.transform.smoothscale(icon, (settings.intro_icons_scale, settings.intro_icons_scale))
        self.screen.blit(icon, (self.xmax - int(self.xmax*170/1280), self.ymin - int(self.ymax*75/800)))


        icon = disputil.loadIcon("Virus3_Intro", settings.icons_folder)
        icon = pygame.transform.smoothscale(icon, (settings.intro_icons_scale, settings.intro_icons_scale))
        self.screen.blit(icon, (self.xmin - int(self.xmax*120/1280), self.ymin - int(self.ymax*80/800)))


        icon = disputil.loadIcon("Virus4_Intro", settings.icons_folder)
        icon = pygame.transform.smoothscale(icon, (settings.intro_icons_scale, settings.intro_icons_scale))
        self.screen.blit(icon, (self.xmax - int(self.xmax*220/1280), self.ymax - int(self.ymax*200/800)))
        ...
        while not done:
            ...
            if first_run:
                ...
                tx, ty = disputil.DrawText(self.screen, settings.options.capitalize(), self.presskey_fontObj,
                                           pygame.Color("white"), blit=False)
                disputil.DrawText(self.screen, settings.options.capitalize(), self.presskey_fontObj, pygame.Color("white"),
                ...
                for i in range(0, len(icon)):
                    icon[i] = disputil.loadIcon("Virus" + str(i+1) +"_Intro", settings.icons_folder)
    ...
    def bacteria_options(self, bacteria):
        ...
        tips = [settings.category_tip, settings.food_tip, settings.starvation_tip, settings.reproduction_tip, None, None, None, None]
        ...
        tx, ty = disputil.DrawText(self.screen, bacteria["Name"], self.name_fontObj, settings.options_title_color,
                                   blit=False)
        disputil.DrawText(self.screen, bacteria["Name"], self.name_fontObj, settings.options_title_color,
        ...
        for i in range(0, len(text)):
            ...
            if tips[i] is not None:
                disputil.DrawText(self.screen, tips[i], self.info_fontObj, settings.options_color,
                ...
                if changed[i]:
                    ...
                    if type(value[i]) is int or type(value[i]) is str:
                        ...
                        if first_run:
                            ...
                            value[i], event_box, input_box = disputil.get_input_box_value(self.screen, rect,
                                                                                          text=str(value[i]))
                    elif type(value[i]) is bool:
                        pygame.draw.rect(self.screen, settings.cBkg_highlighted, (ix, iy, 32, 32))
    ...
    def general_options(self):
        ...
        frame_icon = disputil.loadIcon(settings.frame_icon, settings.icons_folder)
        frame_icon = pygame.transform.smoothscale(frame_icon, (32, 32))
        check_icon = disputil.loadIcon(settings.check_icon, settings.icons_folder)
        check_icon = pygame.transform.smoothscale(check_icon, (32, 32))

        done = False
        text = ["Initial Specimen Number", "Show Icons", "Show Values", "Food Mode Activated", "Food Growth Ratio", "Show Terrain", "Logging"]
        tips = [None, None, None, None, settings.food_growth_tip, None, None]
        ...
        tx, ty = disputil.DrawText(self.screen, settings.options, self.title_fontObj, settings.options_title_color,
                                   blit=False)
        disputil.DrawText(self.screen, settings.options, self.title_fontObj, settings.options_title_color,
        ...
        for i in range(0, len(text)):
            tx, ty = disputil.DrawText(self.screen, text[i], self.presskey_fontObj, settings.options_color,
                                       blit=True, x=x, y=y)
            if tips[i] is not None:
                disputil.DrawText(self.screen, tips[i], self.info_fontObj, settings.options_color,
                                  blit=True, x=x, y=y+ty*0.8)
...
###############################################################################
class Bacteria(threading.Thread):
